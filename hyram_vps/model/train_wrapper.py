...
class HyRAMVPSTrainWrapper(HyRAMVPS):
    def __init__(self, cfg: DictConfig, stage_cfg: DictConfig):
        ...
        self.gt_memory_start_prob = stage_cfg.get('gt_memory_start_prob', 0.0)
        self.gt_memory_end_prob = stage_cfg.get('gt_memory_end_prob', self.gt_memory_start_prob)
        self.gt_memory_decay_iters = stage_cfg.get('gt_memory_decay_iters', 0)
    ...
    def _get_gt_memory_prob(self, data: Dict) -> float:
        if self.gt_memory_decay_iters <= 0:
            return self.gt_memory_start_prob

        progress = min(float(data.get('it', 0)) / self.gt_memory_decay_iters, 1.0)
        return self.gt_memory_start_prob + progress * (
            self.gt_memory_end_prob - self.gt_memory_start_prob)

    def _get_gt_masks(self, cls_gt: torch.Tensor, ti: int, num_objects: int) -> torch.Tensor:
        obj_ids = torch.arange(1, num_objects + 1, device=cls_gt.device).view(1, num_objects, 1, 1)
        return (cls_gt[:, ti] == obj_ids).float()
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            for ti in range(1, seq_length):
                ...
                if ti < (self.seq_length - 1):
                    ...
                    memory_masks = masks
                    gt_memory_prob = self._get_gt_memory_prob(data)
                    if gt_memory_prob > 0 and np.random.rand() < gt_memory_prob:
                        memory_masks = self._get_gt_masks(data['cls_gt'], ti, num_objects)
