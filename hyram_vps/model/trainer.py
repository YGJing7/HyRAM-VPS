...
from hyram_vps.model.train_wrapper import HyRAMVPSTrainWrapper
from hyram_vps.model.utils.parameter_groups import get_parameter_groups
from hyram_vps.model.losses import LossComputer
from hyram_vps.utils.log_integrator import Integrator
from hyram_vps.utils.image_saver import vis
from hyram_vps.utils.logger import TensorboardLogger
from hyram_vps.utils.time_estimator import TimeEstimator
...
class Trainer:
    def __init__(self, cfg: DictConfig, stage_cfg: DictConfig, log: TensorboardLogger, run_path):
        ...
        self.hyram_vps = nn.parallel.DistributedDataParallel(HyRAMVPSTrainWrapper(cfg, stage_cfg).cuda(),
        ...
        self.log.log_string('model_size',
                            str(sum([param.nelement() for param in self.hyram_vps.parameters()])))
        self.log.log_string(
            'number_of_parameters_that_require_gradient',
            str(
                sum([
                    param.nelement()
                    for param in filter(lambda p: p.requires_grad, self.hyram_vps.parameters())
                ])))
        ...
        self.train()
        parameter_groups = get_parameter_groups(self.hyram_vps, stage_cfg, print_log=(local_rank == 0))
    ...
    def do_pass(self, data, it=0):
        ...
        if self.use_amp:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(self.hyram_vps.parameters(), self.clip_grad_norm)
    ...
    def save_weights(self, it, save_copy=False):
        ...
        if save_copy:
            ...
            torch.save(self.hyram_vps.module.state_dict(), model_path)
    ...
    def save_checkpoint(self, it, save_copy=False):
        ...
        checkpoint = {
            'it': it,
            'weights': self.hyram_vps.module.state_dict(),
    ...
    def load_checkpoint(self, path):
        ...
        map_location = 'cuda:%d' % self.local_rank
        self.hyram_vps.module.load_state_dict(weights)
    ...
    def load_weights_in_memory(self, src_dict):
        self.hyram_vps.module.load_weights(src_dict)
    ...
    def weights(self):
        return self.hyram_vps.module.state_dict()

    def train(self):
        self._is_train = True
        self.integrator = self.train_integrator
        self.hyram_vps.train()
        return self
