...
from hyram_vps.model.group_modules import GConv2d
from hyram_vps.utils.tensor_utils import aggregate
from hyram_vps.model.transformer.positional_encoding import PositionalEncoding
from hyram_vps.model.transformer.transformer_layers import *
...
class QueryTransformer(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.num_heads = this_cfg.num_heads
        self.num_queries = this_cfg.num_queries
        summarizer_cfg = model_cfg.object_summarizer
        self.region_mode = summarizer_cfg.get('region_mode', 'foreground_background')
        self.boundary_kernel_size = summarizer_cfg.get('boundary_kernel_size', 3)
    ...
    def _get_region_query_counts(self, num_regions: int):
        base_count = self.num_queries // num_regions
        remainder = self.num_queries % num_regions
        return [base_count + (1 if i < remainder else 0) for i in range(num_regions)]

    def _make_boundary_regions(self, foreground: torch.Tensor):
        batch_size, num_objects, h, w = foreground.shape
        foreground_4d = foreground.float().flatten(0, 1).unsqueeze(1)

        kernel_size = self.boundary_kernel_size
        padding = kernel_size // 2
        dilated = F.max_pool2d(foreground_4d, kernel_size, stride=1, padding=padding)
        eroded = -F.max_pool2d(-foreground_4d, kernel_size, stride=1, padding=padding)

        boundary_in = (foreground_4d - eroded).clamp(0, 1)
        boundary_out = (dilated - foreground_4d).clamp(0, 1)
        boundary = (boundary_in + boundary_out).clamp(0, 1)
        interior = (foreground_4d - boundary_in).clamp(0, 1)
        background = (1 - dilated).clamp(0, 1)

        return [
            boundary.view(batch_size, num_objects, h, w).bool(),
            interior.view(batch_size, num_objects, h, w).bool(),
            background.view(batch_size, num_objects, h, w).bool(),
        ]

    def _regions_to_attention_mask(self, regions):
        region_counts = self._get_region_query_counts(len(regions))
        attention_masks = []
        for region, query_count in zip(regions, region_counts):
            if query_count == 0:
                continue
            blocked = ~region.flatten(start_dim=2)
            blocked = blocked.unsqueeze(2).unsqueeze(2).repeat(
                1, 1, self.num_heads, query_count, 1).flatten(start_dim=0, end_dim=2)
            attention_masks.append(blocked)

        return torch.cat(attention_masks, dim=1)

    def _get_aux_mask(self, logits: torch.Tensor,
                      selector: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        foreground = (logits[:, 1:] >= logits.max(dim=1, keepdim=True)[0]).bool()
        if self.region_mode == 'boundary_interior_background':
            regions = self._make_boundary_regions(foreground)
        elif self.region_mode == 'none':
            regions = [torch.ones_like(foreground)]
        else:
            regions = [foreground, ~foreground]

        aux_mask = self._regions_to_attention_mask(regions)
