...
from hyram_vps.model.modules import *
from hyram_vps.model.big_modules import *
from hyram_vps.model.aux_modules import AuxComputer
from hyram_vps.model.utils.memory_utils import *
from hyram_vps.model.transformer.object_transformer import QueryTransformer
from hyram_vps.model.transformer.object_summarizer import ObjectSummarizer
from hyram_vps.utils.tensor_utils import aggregate

log = logging.getLogger()


class HyRAMVPS(nn.Module):
    ...
    def load_weights(self, src_dict, init_as_zero_if_needed=False) -> None:
        if not self.single_object:
            ...
            k = 'pixel_fuser.sensory_compress.weight'
            if src_dict[k].shape[1] == self.sensory_dim + 2:
                log.warning(f'Converting {k} from multiple objects to single object.'
                            'This is not supposed to happen in standard training.')
                src_dict[k] = src_dict[k][:, :-1]
