...
from hyram_vps.inference.kv_memory_store import KeyValueMemoryStore
...
class MemoryManager:
    ...
    def read(self, pix_feat: torch.Tensor, query_key: torch.Tensor, selection: torch.Tensor,
             last_mask: torch.Tensor, network: HyRAMVPS) -> Dict[int, torch.Tensor]:
        """
        ...
        for bucket_id, bucket in buckets.items():
            ...
            for objects in object_chunks:
                ...
                readout_memory, aux_features = network.readout_query(pixel_readout,
                                                                      this_obj_mem,
                ...
                if self.save_aux:
                    aux_output = {
                        ...
                        'attn_mask': aux_features.get('attn_mask', None).float()
                        if aux_features and aux_features.get('attn_mask', None) is not None else None,
                    }
                    self.aux = aux_output
